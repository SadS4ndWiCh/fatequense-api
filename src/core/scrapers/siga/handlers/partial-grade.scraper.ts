...
import { StudentPartialGrade } from '~/types';
...
export const getStudentPartialGrade = withGXState<StudentPartialGrade>(
  (gxstate) => {
          ...
          examsDates: discipline['Datas'].map((examDate) => ({
            ...
            grade:
              examDate['Avaliacoes'].length > 0
                ? examDate['Avaliacoes'][0][
                    'ACD_PlanoEnsinoAvaliacaoParcialNota'
                  ]
                : 0,
          })),
        })),
