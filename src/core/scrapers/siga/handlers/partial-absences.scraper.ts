import { StudentPartialAbsences } from '~/types';
...
export const getStudentPartialAbsences = withGXState<StudentPartialAbsences>(
  (gxstate) => {
    return studentPartialAbsencesSchema.parse(
      gxstate
        .get('vFALTAS')
        .map((discipline: IDisciplinePartialAbsencesRaw) => ({
