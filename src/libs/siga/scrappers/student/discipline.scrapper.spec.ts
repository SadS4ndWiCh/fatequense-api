...
describe('Discipline scrapper function', () => {
	it('should be able to get student discipline details from HTML', () => {
		...
		const discipline = getDiscipline(extractGXStateOfHTML(disciplineHtmlContent));

		const totalWorkload = discipline.workload.total;
		const expectTotalAbsencesAllowed = totalWorkload - totalWorkload * MINIMUM_ATTENDANCE_PERCENTAGE;
